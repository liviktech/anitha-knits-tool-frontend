...
import { resetPasswordDirect } from './auth-service';

type PageStep = 'mobile' | 'newPassword' | 'success';
...
export function ForgotPasswordPage() {
  ...
  async function handleMobileSubmit(e: FormEvent) {
    ...
    if (!/^[0-9]{10,15}$/.test(trimmed)) {
      setMobileError('Enter a valid 10–15 digit mobile number.');
      return;
    }
  ...
  async function handlePasswordSubmit(e: FormEvent) {
    ...
    try {
      await resetPasswordDirect(mobile, newPassword);
      setPageStep('success');
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Avatar icon */}
        <div
          className="absolute -top-[55px] w-[110px] h-[110px] rounded-full bg-[#071711] border-[2px] border-white/10 shadow-[0_8px_20px_rgba(0,0,0,0.4)] flex items-center justify-center z-20"
          style={{
          ...
          <div className="text-center mb-4">
            ...
            {pageStep === 'newPassword' && (
              <p className="text-white/70 text-[13px] mt-1">
                Setting new password for <span className="font-bold text-white">{mobile}</span>
              </p>
            )}
          </div>
          ...
          {pageStep === 'mobile' && (
            <form onSubmit={handleMobileSubmit} className="w-full flex flex-col">
              <div className="relative w-full">
                <div
                  className={`absolute -top-9 left-0 right-0 w-full flex justify-center items-center border border-red-500 bg-red-100 py-1 overflow-hidden transition-all duration-300 ease-in-out ${
                    mobileError ? 'max-h-12 opacity-100' : 'max-h-0 opacity-0'
                  }`}
                >
                  <p className="text-[15px] font-bold text-red-500 text-center">{mobileError}</p>
                </div>
              ...
              <button
                type="submit"
                className="w-full h-[56px] mt-6 bg-[#053b2a] text-white tracking-[3px] font-bold text-[15px] rounded-[2px] transition-all duration-250 flex items-center justify-center outline-none hover:bg-[#074733] hover:-translate-y-[2px] hover:shadow-[0_4px_15px_rgba(5,59,42,0.4)] cursor-pointer"
              >
                CONTINUE
              </button>
          ...
          {pageStep === 'newPassword' && (
            <form onSubmit={handlePasswordSubmit} className="w-full flex flex-col">
              <div className="relative w-full">
                <div
                  className={`absolute -top-9 left-0 right-0 w-full flex justify-center items-center border border-red-500 bg-red-100 py-1 overflow-hidden transition-all duration-300 ease-in-out ${
                    passwordError ? 'max-h-12 opacity-100' : 'max-h-0 opacity-0'
                  }`}
                >
              ...
              <button
                type="button"
                onClick={() => { setPasswordError(null); setPageStep('mobile'); }}
