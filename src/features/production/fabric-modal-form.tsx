      ...
      <div className="space-y-2 p-3 rounded-lg border border-gray-400">
        ...
        <div className="grid grid-cols-2 gap-4 pt-1">
          <div className="space-y-1.5">
            ...
            <Input type="number" min="0" onKeyDown={(e) => e.key === '-' && e.preventDefault()} min="0" onKeyDown={(e) => e.key === '-' && e.preventDefault()} placeholder="0.00" value={draft.fwKg} onChange={(e) => updateField('fwKg', e.target.value)} />
          </div>
          <div className="space-y-1.5">
            <Label className="text-gray-600 text-xs font-semibold">Bit Waste (kg)</Label>
            <Input type="number" min="0" onKeyDown={(e) => e.key === '-' && e.preventDefault()} min="0" onKeyDown={(e) => e.key === '-' && e.preventDefault()} placeholder="0.00" value={draft.bwKg} onChange={(e) => updateField('bwKg', e.target.value)} />
          </div>
        </div>
      </div>
