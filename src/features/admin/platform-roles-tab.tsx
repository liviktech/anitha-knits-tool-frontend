...
import {
  useCreatePlatformRight,
  useCreatePlatformRoleAccess,
  useDeletePlatformRight,
  useDeletePlatformRoleAccess,
  usePlatformModules,
  usePlatformRights,
  usePlatformRoleAccesses,
  usePlatformTabs,
  useUpdatePlatformRight,
  useUpdatePlatformRoleAccess,
  useAssignPlatformRoleAccess,
  usePlatformEmployeeAccess,
  type PlatformModuleRecord,
  type PlatformRightAction as RightActionValue,
  type PlatformRightRecord,
  type PlatformRoleAccessRecord,
  type PlatformTabRecord,
} from './platform-roles-tab-queries';
...
interface RightFormDialogProps {
  ...
  initial: PlatformRightRecord | null;
  modules: PlatformModuleRecord[];
  tabs: PlatformTabRecord[];
...
interface RoleFormDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  initial: PlatformRoleAccessRecord | null;
  rights: PlatformRightRecord[];
...
function RoleFormDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      setEffectiveDate(initial?.effectiveDate?.slice(0, 10) ?? new Date().toISOString().slice(0, 10));
      setRightIds(initial?.rightIds ?? []);
      setFormError(null);
    }
  }, [open, initial]);

  // One card per module (not per module+tab — LK Space's modules today have no tabs), each
  // listing its rights with a select-all-in-module checkbox and a "N rights" count badge.
  const rightsByModule = useMemo(() => {
    const groups = new Map<string, PlatformRightRecord[]>();
    rights.forEach((right) => {
      const list = groups.get(right.moduleName) ?? [];
      list.push(right);
      groups.set(right.moduleName, list);
    });
  ...
  const isGroupSelected = (groupRights: PlatformRightRecord[]) =>
    groupRights.length > 0 &&
    groupRights.every((right) => rightIds.includes(right.id));

  const isGroupPartiallySelected = (groupRights: PlatformRightRecord[]) =>
    groupRights.some((right) => rightIds.includes(right.id)) &&
    !isGroupSelected(groupRights);

  const toggleGroup = (groupRights: PlatformRightRecord[]) => {
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-3xl border border-gray-400 flex flex-col max-h-[88vh]">
        <DialogHeader className="-mx-4 -mt-4 mb-2 shrink-0 rounded-t-xl border-b border-gray-200 bg-[#A8DCAB] px-4 py-3">
          <DialogTitle className="text-lg font-bold text-black">{isEdit ? 'Edit Role' : 'Add New Role'}</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto pr-1 -mr-1">
          <div className="flex flex-col gap-4 py-2">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="role-name" className="text-xs font-semibold text-gray-700">
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="effective-date" className="text-xs font-semibold text-gray-700">
            ...
            <div>
              <span className="inline-flex items-center rounded-full bg-blue-50 px-3 py-1 text-[11px] font-bold uppercase tracking-wide text-blue-700">
                {rightIds.length} rights selected
              </span>
            </div>

            {rightsByModule.length === 0 ? (
              <p className="rounded-lg border border-gray-200 py-8 text-center text-[13px] text-gray-400">No rights configured yet.</p>
            ) : (
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                {rightsByModule.map(([moduleName, groupRights]) => {
                  const selected = isGroupSelected(groupRights);
                  const partial = isGroupPartiallySelected(groupRights);
                  return (
                    <div key={moduleName} className="rounded-lg border border-gray-200 p-3.5">
                      <div className="mb-2.5 flex items-center justify-between gap-2">
                        <label className="flex min-w-0 cursor-pointer items-center gap-2">
                          <input
                            ...
                            disabled={isPending}
                            className="h-3.5 w-3.5 shrink-0 rounded border-gray-300 text-[#004D40] focus:ring-[#004D40]"
                          />
                          <span className="truncate text-[13px] font-bold text-gray-900">{moduleName}</span>
                        </label>
                        <span className="shrink-0 rounded-full bg-gray-100 px-2 py-0.5 text-[11px] font-medium text-gray-500">
                          {groupRights.length} right{groupRights.length === 1 ? '' : 's'}
                        </span>
                      </div>
                      <div className="flex flex-col gap-1.5">
                        {groupRights.map((right) => (
                          <label key={right.id} className="flex items-center gap-2 text-[13px] text-gray-700">
                            <input
                              type="checkbox"
...
interface AssignRoleDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  roles: PlatformRoleAccessRecord[];
  employees: LivikEmployee[];
  currentRoleNameByEmpId: Map<string, string>;
  onSubmit: (roleAccessId: string, livikEmpIds: string[]) => Promise<boolean>;
  isPending: boolean;
  serverError?: string | null;
}

function AssignRoleDialog({ open, onOpenChange, roles, employees, currentRoleNameByEmpId, onSubmit, isPending, serverError }: AssignRoleDialogProps) {
  ...
  const toggleEmployee = (empId: string) => {
    setSelectedEmpIds((prev) => (prev.includes(empId) ? prev.filter((e) => e !== empId) : [...prev, empId]));
  };
  ...
  const handleSubmit = async () => {
    ...
    setFormError(null);
    const ok = await onSubmit(roleId, selectedEmpIds);
    if (ok) onOpenChange(false);
  };
    ...
    <Dialog open={open} onOpenChange={(next) => { if (!isPending) onOpenChange(next); }}>
      <DialogContent className="sm:max-w-md border border-gray-400">
        ...
        <div className="flex flex-col gap-4 py-2">
          ...
          <div className="flex flex-col gap-1.5">
            <Label className="text-xs font-semibold text-gray-700">Livik Employees</Label>
            <Input
                  ...
                  >
                    <span className="flex items-center gap-2">
                      <input
                        ...
                        onChange={() => toggleEmployee(employee.empId)}
                      ...
                      {employee.designation && (
                        <span className="text-[12px] text-gray-400">{employee.designation}</span>
                      )}
                    </span>
                    {currentRoleNameByEmpId.get(employee.empId) && (
...
export function PlatformRolesTab() {
  ...
  const modulesQuery = usePlatformModules();
  const tabsQuery = usePlatformTabs();
  const rightsQuery = usePlatformRights();
  const roleAccessQuery = usePlatformRoleAccesses();
  const employeesQuery = useLivikEmployees();
  const employeeAccessQuery = usePlatformEmployeeAccess();

  const createRight = useCreatePlatformRight();
  const updateRight = useUpdatePlatformRight();
  const deleteRight = useDeletePlatformRight();

  const createRoleAccess = useCreatePlatformRoleAccess();
  const updateRoleAccess = useUpdatePlatformRoleAccess();
  const deleteRoleAccess = useDeletePlatformRoleAccess();
  const assignRoleAccess = useAssignPlatformRoleAccess();
  ...
  const [editingRole, setEditingRole] = useState<PlatformRoleAccessRecord | null>(null);
  const [deleteRoleTarget, setDeleteRoleTarget] = useState<PlatformRoleAccessRecord | null>(null);

  const [isAssignDialogOpen, setIsAssignDialogOpen] = useState(false);

  const [isRightDialogOpen, setIsRightDialogOpen] = useState(false);
  const [editingRight, setEditingRight] = useState<PlatformRightRecord | null>(null);
  const [deleteRightTarget, setDeleteRightTarget] = useState<PlatformRightRecord | null>(null);
  ...
  const employeeAccess = employeeAccessQuery.data ?? [];

  const currentRoleNameByEmpId = useMemo(() => {
  ...
  const roleEmployeeCounts = useMemo(() => {
    ...
    employeeAccess.forEach((row) => {
      if (row.isActive && row.roleAccessId) counts.set(row.roleAccessId, (counts.get(row.roleAccessId) ?? 0) + 1);
    });
    return counts;
  }, [employeeAccess]);
  ...
  const handleOpenEditRole = (role: PlatformRoleAccessRecord) => { setEditingRole(role); setIsRoleDialogOpen(true); };

  const handleAssignSubmit = (roleAccessId: string, livikEmpIds: string[]) =>
    assignRoleAccess.mutate({ roleAccessId, livikEmpIds });

  const handleRoleSubmit = (data: { roleName: string; description: string; effectiveDate: string; rightIds: string[] }) =>
    editingRole
  ...
  const handleOpenEditRight = (right: PlatformRightRecord) => { setEditingRight(right); setIsRightDialogOpen(true); };
  ...
  const isLoading = modulesQuery.isLoading || tabsQuery.isLoading || rightsQuery.isLoading || roleAccessQuery.isLoading || employeesQuery.isLoading || employeeAccessQuery.isLoading;
  const isError = modulesQuery.isError || tabsQuery.isError || rightsQuery.isError || roleAccessQuery.isError || employeesQuery.isError || employeeAccessQuery.isError;
  ...
  return (
    <div className="flex flex-col h-full bg-[#004D40]/5 min-h-full flex-1">
      <div className="px-4 py-3 bg-[#F4F1E8] border-b border-[#004D40] shrink-0">
        <h1 className="text-[20px] font-bold text-black leading-tight px-2">Roles and Rights</h1>
        <p className="text-[12.5px] text-gray-500 font-medium px-2">Manage LK Space roles and module permissions</p>
      </div>
      ...
      <AssignRoleDialog
        ...
        roles={roles}
        employees={employees}
        currentRoleNameByEmpId={currentRoleNameByEmpId}
