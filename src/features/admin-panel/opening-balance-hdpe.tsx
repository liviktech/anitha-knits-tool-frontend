...
export function OpeningBalanceHDPETab() {
  ...
  const [records, setRecords] = useState<OpeningBalanceHDPEGroup[]>([]);
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-medium text-gray-900">Raw Materials (HDPE, Chemicals, Colors) Opening Balance</h3>
      ...
      {isModalOpen && (
        <HDPEMOdal
          ...
          onSave={(data) => {
            if (editingRecord) {
              setRecords(records.map(r => r.id === editingRecord.id ? { ...r, ...data } : r));
            } else {
              setRecords([...records, { ...data, id: Date.now().toString() }]);
            }
...
function HDPEMOdal({ onClose, onSave, initialData, lookupsData }: { onClose: () => void; onSave: (data: any) => void; initialData: OpeningBalanceHDPEGroup | null; lookupsData: any; }) {
  ...
  const hdpeNames = (lookupsData?.brands ?? []).map((b: any) => b.name).sort();
  const chemicalNames = (lookupsData?.chemicals ?? []).map((c: any) => c.name).sort();
  ...
  const handleSubmit = () => {
    ...
    const parsedHdpe: Record<string, { bags?: number; weightKg?: number }> = {};
    hdpeNames.forEach((n: string) => {
      const b = parseInt(hdpeBags[n]);
      const w = parseFloat(hdpeWeights[n]);
      if (!isNaN(b) || !isNaN(w)) {
        parsedHdpe[n] = {};
        if (!isNaN(b)) parsedHdpe[n].bags = b;
        if (!isNaN(w)) parsedHdpe[n].weightKg = w;
      }
    });

    const parsedChem: Record<string, number> = {};
    chemicalNames.forEach((n: string) => {
      const w = parseFloat(chemWeights[n]);
      if (!isNaN(w)) parsedChem[n] = w;
    });

    const parsedCol: Record<string, number> = {};
    colorNames.forEach((n: string) => {
      const w = parseFloat(colorWeights[n]);
      if (!isNaN(w)) parsedCol[n] = w;
    });

    onSave({
      date: date.toISOString(),
      hdpe: parsedHdpe,
      chemicals: parsedChem,
      colors: parsedCol,
    });
  };
          ...
          <div className="flex justify-center w-full">
            <div className="border border-gray-200 rounded-lg overflow-x-auto shadow-sm w-max max-w-[95vw]">
              <table className="text-sm border-collapse bg-white">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-300">
                    {hdpeNames.length > 0 && <th colSpan={hdpeNames.length * 2} className="px-3 py-2 text-center font-bold text-blue-700 uppercase tracking-wide bg-blue-50/50 border-r border-gray-200">HDPE</th>}
